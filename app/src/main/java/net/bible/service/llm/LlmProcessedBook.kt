...
class LlmProcessedBackend(
    ...
    private fun resolveEffectiveModel(state: LlmProcessedBackendState): String {
        return try {
            LlmProcessingService.resolveFromConfig(state.llmConfig).model
        } catch (_: Exception) {
            "unknown"
        }
    }
...
private fun createProcessedMetadata(
    ...
    val llmConfigStr = if (llmConfig != null && (llmConfig.providerConfigId != null || llmConfig.model != null)) {
...
val Book.llmEffectiveModel: String
    get() {
        return try {
            LlmProcessingService.resolveFromConfig(llmModelConfig).model
        } catch (_: Exception) {
            "unknown"
        }
    }
