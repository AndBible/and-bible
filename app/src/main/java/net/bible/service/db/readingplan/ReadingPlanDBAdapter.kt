...
/**
 * @author Timmy Braun (2/14/2019) [tim.bze at gmail dot com]
 */
class ReadingPlanDBAdapter {
    companion object {
        const val TAG = "ReadingPlanDBAdapter"
        val app = BibleApplication.application
        val dbHelper = CommonDatabaseHelper.getInstance()
        val VERSIFICATION = "Versification"
        val DEFAULT_VERSIFICATION = SystemKJV.V11N_NAME
        val INCLUSIVE_VERSIFICATION = SystemNRSVA.V11N_NAME
        ...
        private val displayDateFormat = SimpleDateFormat.getDateInstance()

    }

    var metaCurrentActiveReadingPlanID: Int?
        get() {
            val projection = arrayOf(BaseColumns._ID)
            val selection = "${ReadingPlanMeta.COLUMN_LAST_USED_PLAN}=?"
            val selectionArgs = arrayOf(DB_TRUE_VALUE)
    ...
    fun getMetaCurrentDayNumber(readingPlanMetaId: Int): Int {
        if (getMetaIsDateBasedPlan(readingPlanMetaId)) {
            ...
            val projection: Array<String> = arrayOf(ReadingPlanDays.COLUMN_DAY_NUMBER)
            val selection: String = "${ReadingPlanDays.COLUMN_READING_PLAN_META_ID}=? AND ${ReadingPlanDays.COLUMN_READING_DATE}=?"
            val selectionArgs: Array<String> = arrayOf(readingPlanMetaId.toString(), dateFormatterPlanDateToString(today))
    ...
    fun setMetaCurrentDayNumber(readingPlanMetaId: Int, dayNumber: Int) {
        if (!getMetaIsDateBasedPlan(readingPlanMetaId)) {
            ...
            dbWritable.update(ReadingPlanMeta.TABLE_NAME, values, whereClause, whereArgs)
        }
    }

    fun getDueDayToBeRead(readingPlanMetaId: Int): Long {
        val today = CommonUtils.getTruncatedDate()
        val startDate = getMetaReadingPlanStartDate(readingPlanMetaId)
        startDate ?: return -1
    ...
    fun incrementCurrentPlanDay(): Int {
        val readingPlanMetaId: Int = metaCurrentActiveReadingPlanID!!
        val currentDay: Int = getMetaCurrentDayNumber(readingPlanMetaId)

        val projection = arrayOf(ReadingPlanDays.COLUMN_DAY_NUMBER)
        val selection = "${ReadingPlanDays.COLUMN_READING_PLAN_META_ID}=? AND ${ReadingPlanDays.COLUMN_DAY_NUMBER}>?"
        val selectionArgs = arrayOf(readingPlanMetaId.toString(), currentDay.toString())

        val q = dbReadable.query(ReadingPlanDays.TABLE_NAME, projection, selection, selectionArgs,null,null,ReadingPlanDays.COLUMN_DAY_NUMBER)
        var result: Int = 0
        if (q.moveToFirst()) {
            result = q.getInt(0)
            q.close()
        ...
        if (result > 0) {
            setMetaCurrentDayNumber(metaCurrentActiveReadingPlanID!!, result)
        }
    ...
    fun setAllDaysReadUpTo(dayNumber: Int) {
        ...
        val values = ContentValues().apply {
            put(ReadingPlanDays.COLUMN_READ_STATUS,
                ReadingPlanOneDayDB.ReadingStatus(readingPlanMetaId, numberOfReadings,getDayReadingChaptersArrayAsAllRead(numberOfReadings)).toJsonString()
            )
        }
        ...
        dbWritable.update(ReadingPlanDays.TABLE_NAME, values, whereClause, whereArgs)
    }

    fun getMetaIsDateBasedPlan(ReadingPlanMetaID: Int): Boolean {
    ...
    fun getMetaIsCustomUserAddedPlan(fileName: String): Boolean {
        return when (getPlanDetailLink(null, fileName)) {
            null -> true
            else -> false
        }
    }

    fun getMetaIsPlanAlreadyImported(fileName: String): Boolean {
        val q = dbReadable.query(
            ReadingPlanMeta.TABLE_NAME,
     ...
     * from DB with second of this function with double constructor, here: [getMetaReadingPlanDescription]
     */
    fun getMetaReadingPlanDescription(ReadingPlanMetaID: Int): String {
        return getPlanDetailLink(ReadingPlanMetaID)?.planDescription ?: getMetaReadingPlanDescription(ReadingPlanMetaID, true)
    }
     ...
     * constructor param does, here: [getMetaReadingPlanDescription]
     */
    private fun getMetaReadingPlanDescription(ReadingPlanMetaID: Int, fromDB: Boolean): String {
        return getMetaStringDB(ReadingPlanMetaID, ReadingPlanMeta.COLUMN_DESCRIPTION) ?: ""
    }

    fun getMetaFileNameFromId(ReadingPlanMetaID: Int): String {
        return getMetaStringDB(ReadingPlanMetaID, ReadingPlanMeta.COLUMN_PLAN_FILE_NAME) ?: ""
    }

    fun getMetaReadingPlanList(): List<ReadingPlanInformationDB> {
        val readingPlanList = ArrayList<ReadingPlanInformationDB>()

        val projection = arrayOf(ReadingPlanMeta.COLUMN_ID)
        val q = dbReadable.query(ReadingPlanMeta.TABLE_NAME, projection,null,null,null,null,null)
        while (q.moveToNext()) {
    ...
    fun getMetaPlanDaysList(readingPlanMetaId: Int): List<ReadingPlanOneDayDB> {
        ...
        val selection = "${ReadingPlanDays.COLUMN_READING_PLAN_META_ID}=?"
        val selectionArgs = arrayOf(readingPlanMetaId.toString())
        val q = dbReadable.query(ReadingPlanDays.TABLE_NAME, projection, selection, selectionArgs,null,null, ReadingPlanDays.COLUMN_DAY_NUMBER)
        while (q.moveToNext()) {
        ...
        Log.d(TAG, "${readingPlanDaysList.count()} days as list returned from plan ${planInfo.readingPlanName}.")
        return readingPlanDaysList
    }

    fun getMetaReadingPlanName(ReadingPlanMetaID: Int): String {
        return getPlanDetailLink(ReadingPlanMetaID)?.planName ?: getMetaReadingPlanName(ReadingPlanMetaID, true)
    }

    fun getMetaReadingPlanName(ReadingPlanMetaID: Int, fromDB: Boolean): String {
        return getMetaStringDB(ReadingPlanMetaID, ReadingPlanMeta.COLUMN_PLAN_NAME) ?: ""
    }

    fun getMetaReadingPlanStartDate(readingPlanMetaId: Int): Date? {
     ...
     */
    fun setMetaReadingPlanStartDate(readingPlanMetaId: Int, startDate: Date? = null) {
        ...
        val rowsAffected = dbWritable.update(ReadingPlanMeta.TABLE_NAME, values, whereClause, whereArgs)
    ...
    fun getMetaVersificationName(ReadingPlanMetaID: Int): String? {
    ...
    fun getDayNumberOfReadings(ReadingPlanMetaID: Int, dayNumber: Int): Int {
    ...
    fun getDayReadingChaptersArrayAsAllRead(numberOfReadings: Int): Array<ReadingPlanOneDayDB.ChapterRead?> {
        val chapterArray: Array<ReadingPlanOneDayDB.ChapterRead?> = arrayOfNulls(numberOfReadings)

        for (i in 0 until numberOfReadings) {
            chapterArray[i] = ReadingPlanOneDayDB.ChapterRead(i + 1,true)
        }
        return chapterArray
    }
    ...
    fun setDayReadingStatus(readingPlanMetaID: Int, dayNumber: Int, readingStatusJson: String) {
        ...
        val whereClause: String = "${ReadingPlanDays.COLUMN_READING_PLAN_META_ID}=? AND ${ReadingPlanDays.COLUMN_DAY_NUMBER}=?"
        val whereArgs = arrayOf(readingPlanMetaID.toString(),dayNumber.toString())
    ...
    fun resetPlan(readingPlanMetaId: Int) {
        ...
        val metaWhereArgs = whereArgs
        dbWritable.update(ReadingPlanMeta.TABLE_NAME, metaValues, metaWhereClause, metaWhereArgs)
        Log.d(TAG, "Updated start date and current day on reset")
    }

    fun switchToReadingPlan(readingPlanInformationParam: ReadingPlanInformationDB) {
        Log.d(TAG, "Switching reading plan. R.Plan.Id=${readingPlanInformationParam.readingPlanMetaID} -- " +
            "startDate=${readingPlanInformationParam.readingPlanStartDate}")
        metaCurrentActiveReadingPlanID = readingPlanInformationParam.readingPlanMetaID
        if (getMetaReadingPlanStartDate(readingPlanInformationParam.readingPlanMetaID) == null) {
            setMetaReadingPlanStartDate(readingPlanInformationParam.readingPlanMetaID)
        }
    }

    private fun getDayIntegerDB(ReadingPlanMetaID: Int, ReadingPlanDayNumber: Int, getColumn: String): Int? {
        val q = dbReadable.query(ReadingPlanDays.TABLE_NAME,
            arrayOf(getColumn),
    ...
    private fun getMetaIntegerDB(ReadingPlanMetaID: Int, getColumn: String, selection: String? = null, selectionArgs: Array<String>? = null): Int? {
        val q = dbReadable.query(ReadingPlanMeta.TABLE_NAME,
            arrayOf(getColumn),
        ...
        var result: Int? = null
        if (q.moveToFirst()) {
            Log.d(TAG, "getMetaIntegerDB rPlanId=$ReadingPlanMetaID -- getColumn=$getColumn -- Result=${q.getInt(0)}")
    ...
    private fun getMetaLongDB(ReadingPlanMetaID: Int, getColumn: String, selection: String? = null, selectionArgs: Array<String>? = null): Long? {
        val q = dbReadable.query(ReadingPlanMeta.TABLE_NAME,
            arrayOf(getColumn),
            selection ?: "${ReadingPlanMeta.COLUMN_ID}=?",
            selectionArgs ?: arrayOf(ReadingPlanMetaID.toString()),
    ...
    private fun getMetaStringDB(ReadingPlanMetaID: Int, getColumn: String): String? {
        val q = dbReadable.query(ReadingPlanMeta.TABLE_NAME, arrayOf(getColumn),"${ReadingPlanMeta.COLUMN_ID}=?", arrayOf(ReadingPlanMetaID.toString()),null,null,null)
        var result: String? = null
        if (q.moveToFirst()) {
            result = q.getString(0)
        }
     ...
     */
    private fun getPlanDetailLink(readingPlanMetaID: Int? = null, fileName: String? = null): DailyReading.Companion.PlanDetails? {
        val useFileName = fileName ?: getMetaFileNameFromId(readingPlanMetaID!!)
        val planDetailLink: DailyReading.Companion.PlanDetails? = DailyReading.ABDistributedPlanDetailArray.find { it.fileName == useFileName }
        return planDetailLink
    }
...
class ReadingPlanInformationDB(private val ReadingPlanMetaIdParam: Int?) {
    ...
    val readingPlanMetaID: Int = ReadingPlanMetaIdParam ?: dbAdapter.metaCurrentActiveReadingPlanID!!
    val readingPlanIsDateBased: Boolean = dbAdapter.getMetaIsDateBasedPlan(readingPlanMetaID)
    val readingPlanFileName: String = dbAdapter.getMetaFileNameFromId(readingPlanMetaID)
    val readingPlanName: String = dbAdapter.getMetaReadingPlanName(readingPlanMetaID)
    val readingPlanDescription: String = dbAdapter.getMetaReadingPlanDescription(readingPlanMetaID)
    val readingPlanStartDate: Date? = dbAdapter.getMetaReadingPlanStartDate(readingPlanMetaID)
    val readingPlanTotalDays: Int = dbAdapter.getMetaTotalDays(readingPlanMetaID)
    val readingPlanVersificationName: String? = dbAdapter.getMetaVersificationName(readingPlanMetaID)
    val readingPlanIsCustomUserAdded: Boolean = dbAdapter.getMetaIsCustomUserAddedPlan(readingPlanFileName)
    ...
    fun getReadingPlanVersification(): Versification? {
        if (readingPlanVersification == null) {
            try {
                readingPlanVersification = Versifications.instance().getVersification(readingPlanVersificationName)
            } catch (e: Exception) {
                Log.e(TAG, "Error loading versification from Reading plan:$readingPlanFileName. Will now use ${dbAdapterStatic.INCLUSIVE_VERSIFICATION} because it includes most books.")
                readingPlanVersification = Versifications.instance().getVersification(dbAdapterStatic.INCLUSIVE_VERSIFICATION)
            }
        }
...
class ReadingPlanOneDayDB(private val readingPlanInformationParam: ReadingPlanInformationDB?,
    ...
    var readingPlanInfo: ReadingPlanInformationDB = readingPlanInformationParam ?: ReadingPlanInformationDB(readingPlanMetaId)

    init {
        if (readingPlanMetaId == null) {
            readingPlanMetaId = readingPlanInfo.readingPlanMetaID
            dayNumber = readingPlanDayNumberParam ?: dbAdapter.getMetaCurrentDayNumber(readingPlanMetaId!!)
    ...
    private fun calculateReadingDateString(): String {
        var dateString = ""
        val startDate = readingPlanInfo.readingPlanStartDate
        if (startDate != null) {
    ...
    private fun generateReadingKeys(): ArrayList<Key>? {
        if (readingChaptersString != null) {
            ...
            if (StringUtils.isNotEmpty(readingChaptersString)) {
                ...
                val readingArray = readingChaptersString!!.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    data class ReadingStatus(val readingPlanMetaId: Int,
                             ...
                             var chapterReadArray: Array<ChapterRead?>?
        ...
        init {
            if (chapterReadArray == null) {
                ...
                chapterReadArray = arrayOfNulls<ChapterRead>(chapterReadArrayList.size)
                chapterReadArrayList.toArray(chapterReadArray)
            }
        }
