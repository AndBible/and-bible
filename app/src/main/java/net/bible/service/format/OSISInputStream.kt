/*
 * Copyright (c) 2020 Martin Denham, Tuomas Airaksinen and the And Bible contributors.
 *
 * This file is part of And Bible (http://github.com/AndBible/and-bible).
 *
 * And Bible is free software: you can redistribute it and/or modify it under the
 * terms of the GNU General Public License as published by the Free Software Foundation,
 * either version 3 of the License, or (at your option) any later version.
 *
 * And Bible is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
 * without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 * See the GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License along with And Bible.
 * If not, see http://www.gnu.org/licenses/.
 *
 */
package net.bible.service.format

import net.bible.service.common.Logger
import org.apache.commons.lang3.StringUtils
import org.crosswire.jsword.book.Book
import org.crosswire.jsword.book.BookException
import org.crosswire.jsword.passage.Key
import java.io.IOException
import java.io.InputStream
import java.io.UnsupportedEncodingException

/** Read through the raw OSIS input from a bible, add verse tags if required, remove any extra div tags,
 * and pipe back as in InputStream ready to be fed to the SAXParser for html formatting.
 * This is more efficient than using JDom to create a DOM and then streaming the DOM into a SAX parser
 *
 * @author Martin Denham [mjdenham at gmail dot com]
 */
class OSISInputStream(// requested passage
    private val book: Book, key: Key) : InputStream() {

    // iterator
    private var isFirstVerse = true
    private val keyIterator: Iterator<Key> = key.iterator()
    private var isClosingTagWritten = false

    // allow avoidance of repeated text due to merged verses
    private var previousVerseRawText = ""

    // cache
    private lateinit var verseBuffer: ByteArray
    private var length = 0
    private var next = 0
    private val osisVerseTidy: OSISVerseTidy = OSISVerseTidy(book)

    /* (non-Javadoc)
	 * @see java.io.InputStream#read()
	 */
    @Throws(IOException::class)
    override fun read(): Int {
        return if (available() > 0) {
            verseBuffer[next++].toInt()
        } else -1
    }

    /* (non-Javadoc)
	 * @see java.io.InputStream#available()
	 */
    @Throws(IOException::class)
    override fun available(): Int {
        if (next >= length) {
            loadNextVerse()
        }
        return length - next
    }

    /* (non-Javadoc)
	 * @see java.io.InputStream#read(byte[], int, int)
	 */
    @Throws(IOException::class)
    override fun read(b: ByteArray, bOffset: Int, lenToRead: Int): Int {
        if (bOffset < 0 || bOffset > b.size || lenToRead < 0
            || bOffset + lenToRead > b.size || bOffset + lenToRead < 0) {
            throw IndexOutOfBoundsException()
        } else if (lenToRead == 0) {
            return 0
        }
        val available = available()
        // have we reached the end of the chapter
        if (available == 0) {
            return -1
        }
        val lenActuallyCopied = Math.min(available, lenToRead)
        System.arraycopy(verseBuffer, next, b, bOffset, lenActuallyCopied)
        next += lenActuallyCopied
        return lenActuallyCopied
    }

    /** load the next verse, or opening or closing <div> into the verse buffer
     *
     * @throws UnsupportedEncodingException
    </div> */
    @Throws(UnsupportedEncodingException::class)
    private fun loadNextVerse() {
        try {
            if (isFirstVerse) {
                putInVerseBuffer(DOC_START)
                isFirstVerse = false
                return
            }
            var isNextVerseLoaded = false
            while (keyIterator.hasNext() && !isNextVerseLoaded) {
                val currentVerse = keyIterator.next()
                //get the actual verse text and tidy it up,
                var rawText: String
                rawText = try {
                    book.getRawText(currentVerse)
                } catch (e: BookException) {
                    if (e.message!!.startsWith("Unable to obtain raw content")) {
                        ""
                    } else {
                        throw e
                    }
                }

                // do not output empty verses (commonly verse 0 is empty)
                if (!StringUtils.isWhitespace(rawText)) {

                    // merged verses can cause duplicates so if dup then skip immediately to next verse
                    if (previousVerseRawText != rawText) {
                        val tidyText = osisVerseTidy.tidy(currentVerse, rawText)
                        putInVerseBuffer(tidyText)
                        previousVerseRawText = rawText
                        isNextVerseLoaded = true
                        return
                    } else {
                        log.debug("Duplicate verse:$currentVerse")
                    }
                } else {
                    log.debug("Empty or missing verse:$currentVerse")
                }
            }
            if (!isClosingTagWritten) {
                putInVerseBuffer(DOC_END)
                isClosingTagWritten = true
            }
        } catch (usc: UnsupportedEncodingException) {
            usc.printStackTrace()
        } catch (usc: BookException) {
            usc.printStackTrace()
        }
    }

    /** put the text into the verse buffer ready to be fed into the stream
     *
     * @param text
     * @throws UnsupportedEncodingException
     */
    @Throws(UnsupportedEncodingException::class)
    private fun putInVerseBuffer(text: String) {
        verseBuffer = text.toByteArray(charset("UTF-8"))
        length = verseBuffer.size
        next = 0
    }

    companion object {
        //todo get the proper ENTITY refs working rather than my simple 2 fixed entities
        //	private static final String DOC_START = "<!ENTITY % HTMLlat1 PUBLIC \"-//W3C//ENTITIES Latin 1 for XHTML//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml-lat1.ent\">\n%HTMLlat1;\n<div>";
        private const val DOC_START = "<!DOCTYPE div [<!ENTITY nbsp \"&#160;\"><!ENTITY copy \"&#169;\">]><div>"
        private const val DOC_END = "</div>"
        private const val TAG = "OSISInputStream"
        private val log = Logger(TAG)
    }
}
